...
func (c *Converter) Convert(bc *buildv1.BuildConfig) ([]unstructured.Unstructured, Outcome) {
	...
	cmdNS := commandArg(bc.Namespace, namespacePlaceholder, validation.IsDNS1123Label)
	cmdSA := commandArg(bc.Spec.ServiceAccount, serviceAccountPlaceholder, validation.IsDNS1123Subdomain)
	c.warnInvalidName(bc, "namespace", bc.Namespace, cmdNS)
	c.warnInvalidName(bc, "ServiceAccount", bc.Spec.ServiceAccount, cmdSA)
	var cmdSecret string
	if pullSecret != nil {
		cmdSecret = commandArg(pullSecret.Name, pullSecretPlaceholder, validation.IsDNS1123Subdomain)
		c.warnInvalidName(bc, "pull secret", pullSecret.Name, cmdSecret)
	}
...
// warnInvalidName warns when cmdArg (commandArg's result for value) differs
// from value: the target cluster cannot hold an object with that name, and
// cmdArg has taken its place in any command a warning prints. Callers pass
// the already-computed commandArg result because they need that same value
// for the command text nearby, so this does not run the check again.
func (c *Converter) warnInvalidName(bc *buildv1.BuildConfig, what, value, cmdArg string) {
	if cmdArg == value {
		return
	}
...
func (c *Converter) processResources(bc *buildv1.BuildConfig, b *shipwrightv1beta1.Build, generatedSA string) error {
	...
	if b.Spec.Source != nil && b.Spec.Source.Type == shipwrightv1beta1.LocalType {
		...
		if saName != "" {
			...
			cmdSA := commandArg(saName, serviceAccountPlaceholder, validation.IsDNS1123Subdomain)
