...
func dispositionPatch(u unstructured.Unstructured, outcome Outcome) (jsonpatch.Patch, error) {
	...
	if len(u.GetAnnotations()) == 0 {
		...
		// Deterministic order so the emitted patch is stable across runs.
		for _, key := range []string{ConversionOutcomeAnnotation, ConversionReasonAnnotation} {
...
// a rune boundary so the annotation value stays valid UTF-8.
func truncateReason(reason string) string {
	if len(reason) <= maxConversionReasonBytes {
		return reason
	}
	truncated := reason[:maxConversionReasonBytes]
