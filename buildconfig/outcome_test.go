...
// buildConfigRequest wraps a BuildConfig spec (a JSON object string) into a
// plugin request.
func buildConfigRequest(spec string) transform.PluginRequest {
	specMap := map[string]interface{}{}
	if err := json.Unmarshal([]byte(spec), &specMap); err != nil {
		panic("buildConfigRequest: invalid spec JSON: " + err.Error())
	}
...
func TestRunDoesNotAbortOnConversionFailure(t *testing.T) {
	...
	resp, err := plugin.Run(buildConfigRequest(`{
		"strategy": {"type": "Bogus"},
		"output": {"to": {"kind": "DockerImage", "name": "quay.io/example/app:latest"}}
	}`))
	if err != nil {
...
func TestRunConvertsValidBuildConfig(t *testing.T) {
	...
	resp, err := plugin.Run(buildConfigRequest(convertedDockerBC))
	if err != nil {
