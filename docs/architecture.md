...
| 17 | Resources | `processResources` | `spec.resources` | a BuildRun template with the requests and limits, stored as an annotation. Never a live BuildRun. Under a `default-build-strategy` override the template has no `stepResources`, and a warning asks the user to add them. With a Local source the template is still written, but the warning says `shp build upload` cannot use it and the build runs with the strategy's default step resources | template cannot be marshalled: failed |
