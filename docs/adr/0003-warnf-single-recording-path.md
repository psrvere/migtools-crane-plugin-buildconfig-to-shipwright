...
logs it at WARN. The two places that log at ERROR instead call `recordWarning` directly so
they still count. `Convert` writes the warnings annotation exactly once, from the same list
