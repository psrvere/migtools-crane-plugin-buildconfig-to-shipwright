...
Warnings are quoted in the [Warning reference](#warning-reference) at the end, keyed [W1](#w1) to [W82](#w82). Each number is an anchor: `#w12` jumps to [W12](#w12).
...
| `sourceStrategy.env[]` | Converted | `spec.env[]`, and `spec.paramValues[build-env]` as one `NAME=$(NAME)` item per entry | Nothing, unless a name cannot be passed that way (W81) or an entry reads a field (W82) | The kubelet fills each `$(NAME)` from `spec.env` when the build starts, so ConfigMap, Secret and `$(OTHER)` entries reach s2i as they did on OpenShift. s2i writes them into the image as `ENV`, Secret values included, as OpenShift did. [W81](#w81), [W82](#w82) |
...
| <a id="w81" name="w81"></a>W81 | `BuildConfig …/… sets sourceStrategy.env …, and a name like that cannot be passed to s2i as NAME=$(NAME). The entries stay in spec.env, which s2i does not see. Rename them, or add each one to the Build's build-env parameter as NAME=VALUE. For an entry read from a Secret that puts the value on the Build in plain text, so rename it instead` |
| <a id="w82" name="w82"></a>W82 | `BuildConfig …/… sets sourceStrategy.env … from a fieldRef or resourceFieldRef. OpenShift read these from the Build object. Shipwright reads them from the BuildRun pod, so metadata.name and labels give the pod's values. Check that the build still gets the value it expects` |
