...
| `source.binary` without `asFile` (started with `oc start-build --from-dir`, `--from-archive` or `--from-repo`) | Converted, with a warning | `spec.source: {type: Local, local: {name: local-copy, timeout: 10m}}` | Start each build with `shp build upload <build> <directory>`. The directory becomes the build context, as with `--from-dir` | Nothing feeds a Local source on its own. A BuildRun started without the upload waits out the timeout and fails. [W67](#w67) |
