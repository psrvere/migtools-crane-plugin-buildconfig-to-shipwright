...
| <a id="w78" name="w78"></a>W78 | `Volume … was converted. Unlike other strategy volumes, the shipped buildah and source-to-image ClusterBuildStrategies already declare an overridable volume by this name, from strategy-catalog commit cb2432c onward, so a target at or after that commit needs no strategy change — check first: oc get clusterbuildstrategy … -o jsonpath='{.spec.volumes[*].name}'. A catalog older than cb2432c, or a strategy of your own, declares no such volume and Shipwright will reject the Build (Registered=False, reason: UndefinedVolume) until you add one: volumes: [{name: …, overridable: true, emptyDir: {}}] plus a volumeMount for '…' on the strategy build step (…).` |
