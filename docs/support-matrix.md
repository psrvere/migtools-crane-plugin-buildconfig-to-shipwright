...
| `metadata.name`, `metadata.namespace` | Converted | `metadata.name`, `metadata.namespace` | Nothing | none, or [W75](#w75) when the namespace is not a valid Kubernetes name |
...
| <a id="w75" name="w75"></a>W75 | `BuildConfig …/… names … …, which is not a valid Kubernetes name, so the target cluster will reject an object with that name. Any command in this Build's warnings shows … in its place: correct the name in the export and convert again.` |
