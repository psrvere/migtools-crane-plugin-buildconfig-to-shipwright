...
on Opus. A built-in Skill forks on the model of whoever invokes it, so `/tech-review` never
invokes one from the session: it forks `/code-review` at low effort from inside an Opus
sub-agent, and runs its simplify and docs passes as Opus sub-agents.
