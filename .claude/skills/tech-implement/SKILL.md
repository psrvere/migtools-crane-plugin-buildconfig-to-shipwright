...
Before showing the user any summary, question, report, PR body, or completion banner, pass
the text through the `/unslop` skill so it reads like a person wrote it. This covers the
Phase 0 "Ready to proceed?" summary, every `AskUserQuestion` prompt and option, the Phase 8
PR body, and the final Compliance Report / Completion Status. Commit messages are already
unslopped per `AGENTS.md`; this extends the same bar to everything the user reads.
...
```
   ...
   if this machine has no signing key configured, drop `-S` and keep `-s`. For the trailer,
   follow the global `CLAUDE.md` `Co-Authored-By` convention. Write the message through
...
```
   ...
   If the rebase reports conflicts, **stop**. Do not push a worktree that holds conflict
...
Jira:    not updated by this skill — user records the outcome separately
...
### Handoff for the user's Jira update (optional)

Because the user records the outcome themselves, leave everything they need ready to paste:
the results file path, the branch (`<fork-remote>:BUILD-XXXX-<slug> (<repo-name>)`, one line
per repo), the tested state from the gate above, and — when a cluster case is still pending —
which acceptance criteria remain unverified. Do not run any `jira` write command.
...
executed by the shell. Run the body through `/unslop` before writing the file.
...
This skill does not touch Jira. After the PR is open, hand the PR URL back to the user so
they can add it to the issue themselves.
...
| 6 commit + rebase + push to fork | ✅ / ❌ | ahead/behind vs origin/main; `show --name-only` output |
...
End with one of:
- **DONE** — implemented, tested with evidence, pushed (Jira is the user's to update)
